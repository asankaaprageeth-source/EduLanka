...
exports.getTeachers = async (req, res) => {
  try {
    const institute_id = req.user.institute_id;
    const [teachers] = await pool.execute(
      `SELECT u.id, u.name, u.email, u.phone, u.photo, u.created_at,
       GROUP_CONCAT(c.name SEPARATOR ', ') as classes
       FROM users u
...
exports.createClass = async (req, res) => {
  try {
    const institute_id = req.user.institute_id;
    const { name, grade, subject, monthly_fee, schedule, description, teacher_id } = req.body;

    const [result] = await pool.execute(
      'INSERT INTO classes (institute_id, teacher_id, name, grade, subject, monthly_fee, schedule, description) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
      [institute_id, teacher_id || null, name, grade || null, subject || null, monthly_fee || 0, schedule || null, description || null]
    );
